...
- [x] 에러코드를 모아서 볼 수 있게 enum 클래스로 묶기
- [x] 각 도메인별 예외처리 추가 & 테스트코드
- [ ] 엣지케이스 생각 후, 도입
...
### 수정사항
- [x] 프로모션 기간 확인 필요.
- [x] payment에서 결제 로직이 이상하다. 모두 변경해야 할 듯 하다.
- [x] 프로모션 제안으로 인해 추가한 상품이 주문상품에 이상하게 추가가 된다. 수정해야 한다.
- [x] 그리고 "그래도 구매하시겠습니까" 질문에서 N을 입력했을 때, 프로모션 수량까지만 주문에 추가한다.
- [x] 멤버십 할인 최대치를 8000원으로 한정해야 한다.
